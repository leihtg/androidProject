 ...
 * K线中的均线图
 */
public class VolMAChart extends ChartBase {
    /**
     * 构造方法
     */
    public VolMAChart() {
            ...
            for (int index = startNum; index < kCollection.size(); index++) {
                ...
                for (int i = 0; i < get_ParameterList().size(); i++) {
                    if (get_ParameterList().get(i).getValue() > 0) {
                        ...
                        //ComputeList--Ma5的值
                        //添加value值
                        double value = kline.getVolume();

                        if (index >= type-1) {
                            double sumClose = 0.0;
                            for (int j = index -(type-1); j <= index; j++) {
                                sumClose = sumClose + kCollection.get(j).getVolume();
                            }
                            value = sumClose / type;
                        ...
                        word.setDrawColor(outResult.getColor());
