 ...
 * K线中的均线图
 */
public class DEAChart extends ChartBase {
    /**
     * 构造方法
     */
    public DEAChart() {
    ...
    public void initParameter() {
        ...
        //均线的颜色列表
        int[] array = {
               KLineColor.ma10Color, KLineColor.ma30Color
        };
                ...
                for (int i = 0; i < get_ParameterList().size(); i++) {
                    ...
                    if (get_ParameterList().get(i).getValue() > 0) {
                        ...
                        if (index > 0) {
                            ...
                            DIFF = EMA12value -EMA26value;
                            double DEAY = ComputeList.get(index-1).get(outResult.getName());
                            value = (2*DIFF +(9-1)*DEAY)/(9+1);
